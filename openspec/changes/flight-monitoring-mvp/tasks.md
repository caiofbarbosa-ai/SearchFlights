...
- [ ] 0.20 Run 3+ consecutive test executions — PARCIAL (06/09 CORRIGIDO): o rótulo CALENDAR_NOT_OPEN dos ciclos 04-06/09 era FALSO — o usuário tem evidências de que jul/2027 abre no Smiles (browser humano); o estado é o FLAG do Akamai (bloqueio silencioso que persiste/renova). Classificador a corrigir (discriminante com busca controle). Ver `poc/reports/smiles_poc_findings_2026-09-02.md` §3.1.3
...
- [x] 12.5 Test Smiles scraper (miles-only and hybrid) — 05/09: ✅ EXTRAÇÃO VALIDADA — bug de wording descoberto via print do usuário: valores são "X milhas por VIAGANTE" e o regex exigia "por passageiro". Corrigido (regex flexível + extração por cartão com companhia): GRU SUCCESS 357.800 pts (Air France). Híbrido Smiles&Money: Fase 5. NOTA 06/09 (correção): o rótulo CALENDAR_NOT_OPEN de ciclos automatizados era FALSO — jul/2027 está aberto no Smiles (evidências do usuário); o estado real é o flag do Akamai
